...
var maxTick_y = 15;
var maxTick_x = 11;
let target = d3.select(".svgContainer");

function addMatrix(jdata) {
    ...
    target_svg.selectAll("line.horizontalGrid").data(yScale.ticks(maxTick_y)).enter()
        .append("line")
        .attr(
            {
    ...
    target_svg.selectAll("line.verticalGrid")
        .data(yScale.ticks(maxTick_y).filter((d) => d <= maxTick_x))
        .enter()
        .append("line")
        .attr(
            {
    ...
    var rects = target_svg.selectAll('rects')
        ...
        .style("width", function (d) {
            let cc = Math.min(d.name.length, 20)
            let w = Math.min.apply(this, [grid_unit_w, (cc * char_width) + 45])
            return w
        })
    ...
    d3.selectAll(".axis , .horizontalGrid, verticalGrid , .legend,text")
        .style("opacity", 0)
        .transition()
        .delay(function (d, i) {
            return 2500 //+ (i*30)
    ...
    d3.selectAll(".circle-plot")
        .style("opacity", 0)
        .transition()
        .delay(function (d, i) {
            return 4500 + (i * ani_intv)
        })
        .duration(function (d, i) {
                return 900
            }
...
function showByDomain(D) {
    if(!D) {
        d3.selectAll(".indicator, .label-plot,.circle-plot")
            .transition()
            .delay(500)
            .style("opacity", 1)
            .duration(500)
    ...
    d3.selectAll(".indicator")
        ...
        .style("opacity",function(d){
            if(d){
                return (d.domain && !D.localeCompare(d.domain) ? 1: 0)
                }
            })
        .duration(500)
    ...
    d3.selectAll(".label-plot")
        .transition()
        .style("opacity",function(d){
            return (d && !D.localeCompare(d.domain) ? 1: 0)
        })
        .duration(500)
